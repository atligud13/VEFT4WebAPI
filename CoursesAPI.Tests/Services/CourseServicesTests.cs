...
namespace CoursesAPI.Tests.Services
{
	...
	public class CourseServicesTests
	{
		...
		{
            var coursesRepository = _mockUnitOfWork.GetRepository<CourseInstance>();

            // TEST 1 - Returns default semester when no semester is given
            // Arrange:
            var courseListFor20153 = coursesRepository.All().Where(x => x.SemesterID == "20153");

            // Act:
            var courses20153 = _service.GetCourseInstancesBySemester();
            ...
            // Arrange:
            var courseListFor20143 = coursesRepository.All().Where(x => x.SemesterID == "20143");
            var courseListFor20163 = coursesRepository.All().Where(x => x.SemesterID == "20163");

            // Act:
            var courses20143 = _service.GetCourseInstancesBySemester("20143");
            var courses20163 = _service.GetCourseInstancesBySemester("20163");

            // Assert:
            Assert.AreEqual(courseListFor20143.Count(), courses20143.Count());
            Assert.AreEqual(courseListFor20153.Count(), courses20153.Count());
            Assert.AreEqual(courseListFor20163.Count(), courses20163.Count());


            // TEST 3 - Name of main teacher is provided if the course has one
            // Arrange:

            // Act:
            var courseHasMainTeacher = courses20153.SingleOrDefault(x => x.CourseInstanceID == COURSEID_VEFT_20153);
            var courseHasNoMainTeacher = courses20163.SingleOrDefault(x => x.CourseInstanceID == COURSEID_VEFT_20163);

            // Assert:
            Assert.IsNotNull(courseHasMainTeacher);
            Assert.AreNotEqual("", courseHasMainTeacher.MainTeacher);
            Assert.AreEqual("", courseHasNoMainTeacher.MainTeacher);
        }
